...
const routerSchema = {
  ...
  properties: {
    agent: {
      type: Type.STRING,
      description:
        "The agent to use. Either 'Search Agent' for queries requiring up-to-date information, or 'General Agent' for general knowledge.",
...
const validationSchema = {
  ...
  properties: {
    confidence: {
      type: Type.NUMBER,
      description:
        "A confidence score between 0.0 and 1.0 for the generated answer's accuracy and relevance.",
    },
    critique: {
      type: Type.STRING,
      description:
        'A brief critique of the answer, highlighting potential inaccuracies or areas for improvement.',
    },
  },
  ...
  try {
    ...
    for (const pattern of PII_PATTERNS) {
      if (pattern.test(query)) {
        throw new Error(
          'Potential PII detected. Please remove personal information from your query.'
        );
      }
    }
      ...
      if (searchResponse.candidates?.[0]?.groundingMetadata?.groundingChunks) {
        sources = searchResponse.candidates[0].groundingMetadata.groundingChunks
          ...
          .map((chunk: any, index: number) => ({
            ...
            snippet:
              chunk.web.snippet ||
              `Content from "${chunk.web.title}" was used to construct the answer.`,
          }));
      }
    } else {
      // General Agent
