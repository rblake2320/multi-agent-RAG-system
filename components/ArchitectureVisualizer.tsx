...
import { AgentIcon, ArrowDownIcon, BrainCircuitIcon, CheckCircleIcon, DatabaseIcon, FeatherIcon, ShieldIcon, ValidationIcon, XCircleIcon } from './icons';
...
const StageIndicator: React.FC<{
    ...
    const statusIcons = {
        pending: null,
        active: <div className="w-3 h-3 bg-brand-primary rounded-full animate-ping absolute -top-1 -right-1"></div>,
    ...
    return (
        <div className={`bg-base-200 p-4 rounded-lg border-2 relative transition-all duration-300 ${statusClasses[status]}`}>
            {statusIcons[status]}
...
export const ArchitectureVisualizer: React.FC<ArchitectureVisualizerProps> = ({ stage, agent, lastActiveStageOnError }) => {
    const getStatus = (targetStage: Stage): 'active' | 'complete' | 'pending' | 'error' => {
        if (stage === Stage.Error && targetStage === lastActiveStageOnError) {
            return 'error';
        }
        
        const stageOrder = [Stage.Idle, Stage.Sanitization, Stage.Routing, Stage.Searching, Stage.Drafting, Stage.Refining, Stage.Validating, Stage.Complete];
    ...
    const isSearchSkipped = agent === 'General Agent' && (stage === Stage.Drafting || stage === Stage.Refining || stage === Stage.Validating || stage === Stage.Complete);
        ...
        <div className="bg-base-200 p-6 rounded-xl shadow-lg border border-base-300 animate-fade-in">
            ...
            <div className="space-y-2">
                ...
                <StageIndicator
                    ...
                    description={isSearchSkipped ? "Not needed for this query" : "Gathering up-to-date information."}
