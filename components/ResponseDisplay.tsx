    ...
    if (state.stage === Stage.Idle) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-center text-content-300 p-8">
          ...
          <p>
            Your AI-generated response will appear here once you submit a query. The system will
            visualize the entire process from routing to final validation.
          </p>
        </div>
      );
    }
    ...
    if (state.stage === Stage.Error) {
      return (
        <div className="p-4 bg-red-900/50 border border-red-500 rounded-lg">
          ...
          <button
            onClick={onReset}
            className="mt-4 px-4 py-2 bg-red-600 hover:bg-red-700 rounded-md text-white font-semibold"
          >
            Try Again
          </button>
        </div>
      ...
      return (
        <div className="animate-fade-in">
          <div className="flex justify-between items-center mb-4">
            ...
            <div
              className={`px-3 py-1 text-sm rounded-full ${confidence > 0.8 ? 'bg-green-500/20 text-green-300' : 'bg-yellow-500/20 text-yellow-300'}`}
            >
              Confidence: {(confidence * 100).toFixed(1)}%
            </div>
          </div>
          <div
            className="prose prose-invert max-w-none text-content-200"
            dangerouslySetInnerHTML={{ __html: text }}
          ></div>

          <h3 className="mt-8 mb-4 text-lg font-semibold text-content-100 border-t border-base-300 pt-4">
            Sources
          </h3>
          <div className="space-y-3">
            {sources.map(source => (
              <a
                ...
                className="block p-4 bg-base-300/50 hover:bg-base-300 rounded-lg transition-colors"
              >
                <div className="font-semibold text-brand-primary flex items-center gap-2">
                  <CitationIcon className="w-4 h-4" />
                  {source.title}
                </div>
                <p className="text-sm text-content-300 mt-1 line-clamp-2">{source.snippet}</p>
              </a>
            ))}
          </div>
        </div>
